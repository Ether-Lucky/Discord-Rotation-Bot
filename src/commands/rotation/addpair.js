...
const { addPair, getPairs, getRotation } = require('../../services/rotationService');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Add a buddy pair to a rotation.')
    .addUserOption(opt => opt.setName('user1').setDescription('First buddy.').setRequired(true))
    .addUserOption(opt => opt.setName('user2').setDescription('Second buddy.').setRequired(true)),
  ...
  async execute(interaction) {
    ...
    const rotation = await pickRotation(interaction, 'addpair', { u1: user1.id, u2: user2.id });
    if (!rotation) return; // picker shown
    ...
    await interaction.editReply(
      `✅ Added pair at position **${pair.position + 1}** in **${rotation.name}**:\n<@${user1.id}> & <@${user2.id}>`
    );
  },
};